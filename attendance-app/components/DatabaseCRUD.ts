...
const insertCourseData = async (db: SQLite.SQLiteDatabase, course_code: string, course_name: string, total_days: number): Promise<void> => {
  try {
    const res = await db.runAsync('INSERT OR IGNORE INTO courses (course_code, course_name, total_days) VALUES (?, ?, ?)', 
      course_code, course_name, total_days);
...
const getAllTimestamps = async (db: SQLite.SQLiteDatabase): Promise<any[]> => {
  try {
    const result = await db.getAllAsync('SELECT * FROM timestamps');
    return result;
  } catch (error) {
...
const clearTable = async (db: SQLite.SQLiteDatabase, tableName: string): Promise<any[]> => {
  try {
    const res = await db.getAllAsync(`DELETE FROM ${tableName}`);
    return res;
  } catch(err) {
